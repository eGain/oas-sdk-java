...
- **🧪 Test Generation**: Comprehensive test suites with multiple languages and frameworks
  ...
  - **Cross-Language**:
    ...
    - **Playwright** API tests (off by default; shaped like [playwright-api-tests](https://github.com/eGainDev/playwright-api-tests); pass `--playwright` to enable)
...
# Generate test suites (Playwright off by default)
java -jar target/oas-sdk-java-2.1-SNAPSHOT.jar tests openapi.yaml -t unit,integration -o ./generated/tests

# Same with Playwright API tests (output under ./generated/tests/playwright/)
java -jar target/oas-sdk-java-2.1-SNAPSHOT.jar tests openapi.yaml -t unit,integration -o ./generated/tests --playwright
...
# Generate everything including Playwright tests
java -jar target/oas-sdk-java-2.1-SNAPSHOT.jar all openapi.yaml -l java -f jersey -o ./generated --playwright
...
public class Example {
    public static void main(String[] args) {
        try {
            ...
            // Playwright is off by default; use TestConfig.builder().playwrightTests(true) or CLI --playwright
            sdk.generateTests(
...
# Playwright is OFF by default; pass --playwright to append Playwright generation
java -jar oas-sdk.jar tests openapi.yaml -t unit,integration,postman -o ./generated-tests

# Enable Playwright generation (appends playwright type; -t playwright alone is stripped without this flag)
java -jar oas-sdk.jar tests openapi.yaml -t unit,integration -o ./generated-tests --playwright

# Playwright only (still requires --playwright)
java -jar oas-sdk.jar tests openapi.yaml -t playwright -o ./generated-tests --playwright
...
- **Playwright API Tests**: Self-contained suite under `<output>/playwright/` aligned with [playwright-api-tests](https://github.com/eGainDev/playwright-api-tests) conventions (language-agnostic; **off by default**)
  ...
  - Flag: `TestConfig.playwrightTests` (default `false`); CLI `--playwright` on `tests` and `all`
...
Playwright generation is off by default. Pass CLI `--playwright` (or set `TestConfig.playwrightTests(true)`), then the SDK writes a suite under `<output>/playwright/` with tag-grouped clients, data-driven `@generated` specs, and JSON fixtures. Install Node 20+, then:
...
Enable with CLI `--playwright` or programmatically:
...
- [x] Enhanced test generation
  ...
  - [x] Playwright API test generation (default-off; `--playwright` to enable; playwright-api-tests conventions)
